...
import { ParsedDetail, OrderType, NFTProtocolType, cacluteBalanceChange, BalanceChange, NFTMessage, NFTDetail, FTDetail, TransactionLike, bn } from "../types"
...
export class Seaport {
    ...
    static parse(typedData: Message): NFTMessage {
        ...
        const parsedOffers = offer.map(_ => Seaport.parseOrder(_));
        const receivedConsiderRation = consideration.filter(_ => _.recipient === order.offerer).map(_ => Seaport.parseOrder(_));
        ...
        for (const recipient of Array.from(otherRecepients)) {
            const matched = consideration.filter(_ => _.recipient === recipient).map(_ => Seaport.parseOrder(_));
